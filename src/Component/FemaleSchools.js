...
class FemaleSchools extends Component {
    render(){
        const { clickevent, currentvalue} = this.props;
        
        const filteredvalue = currentvalue.filter(x =>(x.gender === "female"));

        return(
            <Link to="/details">
            <div className="title-name">Girls Schools</div>
