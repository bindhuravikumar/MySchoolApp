...
class CoedSchools extends Component {
    render(){
        const { clickevent, currentvalue} = this.props;
        
        const filteredvalue = currentvalue.filter(x =>(x.gender === "Both"));

        return(
            <Link to="/details">
            <div className="title-name">Coed Schools</div>
